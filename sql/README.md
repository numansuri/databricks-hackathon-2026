...
| 13 | `13_staff.sql` | `facilities_enrich_staff` (**new**: named-doctor count, 18 specialist-title flags) + bridge `facilities_doctors` (**new**) | enrich 9,989; bridge **13,456** |
| 14 | `14_description.sql` | `facilities_enrich_description` (**new**: text metrics, ownership sector, founding year, hours, accreditation/24x7/bed mining) | 9,989 |
| 09 | `09_facilities_gold.sql` | `facilities_gold` (**rebuilt**) | **9,989** wide (**176 cols**), 1:1 joins |
| 09b | `09b_gold_comments.sql` | column comments on every `facilities_gold` column | 176 comments |
| 15 | `15_data_dictionary.sql` | `facilities_data_dictionary` (one row per gold column) | **176** |
...
after 09b stamps the comments. **176 rows = 176 gold columns; 0 rows have a NULL/empty `description`** (full coverage).
...
**New gold fields (rebuilt 176-col table) — key additions:**
...
**Latest additions (147 → 176 cols) — clinical staff + description:**
...
`desc_accreditation_signal`, `desc_mentions_24x7`, `desc_bed_count`. Plus refreshed cross-WS derived fields
(`emergency_readiness_score`/`_tier`, `needs_outreach`, `is_digitally_active`, `needs_verification`,
`is_contactable_direct`, `year_established_final`/`_source`, `opd_hours_final`, `has_operating_hours`).
...
> changing the raw 41/51 count. The 147 → 176 gold-column growth is all derived signal, not newly-touched
> raw columns.
