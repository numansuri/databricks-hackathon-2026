...
ALTER TABLE workspace.virtue_foundation_enriched.facilities_gold ALTER COLUMN is_teaching_hospital COMMENT 'Teaching-hospital signal mined from facility free text. ~213 facilities.';
ALTER TABLE workspace.virtue_foundation_enriched.facilities_gold ALTER COLUMN is_medical_college_attached COMMENT 'Medical-college-attached signal mined from facility free text. ~433 facilities.';
...
ALTER TABLE workspace.virtue_foundation_enriched.facilities_gold ALTER COLUMN quality_tier COMMENT 'Quality bucket on accreditation_signal_count: high (>=2; 387 rows), medium (=1; 1,629 rows), unknown (=0; 7,973 rows).';
...
ALTER TABLE workspace.virtue_foundation_enriched.facilities_gold ALTER COLUMN clinical_signal_source COMMENT 'Constant ''capability_procedure_text''; flags here are scraped free-text signals, not authoritative (surfaces provenance/uncertainty).';
...
ALTER TABLE workspace.virtue_foundation_enriched.facilities_gold ALTER COLUMN named_doctor_count COMMENT 'Count of DISTINCT ''Dr. Firstname [Lastname...]'' name mentions mined (case-sensitive) from the capability+procedure+description blob via regexp_extract_all. Max observed = 47. Includes some institution false positives (e.g. ''Dr. MGR Medical University'').';
...
ALTER TABLE workspace.virtue_foundation_enriched.facilities_gold ALTER COLUMN has_physician COMMENT 'Blob RLIKE ''general physician|\bphysician\b''. TRUE for 1,029 facilities. Direct specialist-title evidence.';
ALTER TABLE workspace.virtue_foundation_enriched.facilities_gold ALTER COLUMN specialist_domain_count COMMENT 'Sum of the 18 specialist-presence boolean flags cast to int (range 0-18). Max observed = 14.';
ALTER TABLE workspace.virtue_foundation_enriched.facilities_gold ALTER COLUMN has_specialist_evidence COMMENT 'specialist_domain_count > 0. TRUE for 3,405 facilities.';
...
ALTER TABLE workspace.virtue_foundation_enriched.facilities_gold ALTER COLUMN ownership_sector COMMENT 'Mined ownership sector from lower(description)+name text: ''government'' (390; govt/municipal/district/civil/public-sector/railway/esic terms), ''trust_charitable'' (585; trust/charitable/mission/ngo/foundation/seva terms), ''private'' (402; pvt/private/llp/corporate terms), else NULL (8,612). NULL = unknown, never guessed.';
ALTER TABLE workspace.virtue_foundation_enriched.facilities_gold ALTER COLUMN ownership_sector_source COMMENT 'Literal provenance tag ''description_name_text'' for ownership_sector (companion provenance field).';
ALTER TABLE workspace.virtue_foundation_enriched.facilities_gold ALTER COLUMN desc_founding_year COMMENT 'Founding year mined from description via regexp ''(establish(ed)?|since|founded|inception)[^0-9]{0,12}((18|19|20)\d\d)'', clamped 1800..2026 else NULL. Filled for 631 rows; observed range 1836..2025.';
...
ALTER TABLE workspace.virtue_foundation_enriched.facilities_gold ALTER COLUMN desc_bed_count COMMENT 'Low-priority extra bed-count source from description: array_max of regexp_extract_all ''([0-9]{1,4})\s*-?\s*bed(ded|s)?'', clamped 1..4000 else NULL. Filled for 461 rows; observed range 3..2032.';
...
ALTER TABLE workspace.virtue_foundation_enriched.facilities_gold ALTER COLUMN is_digitally_active COMMENT 'TRUE when digital_presence_score >= 0.5 AND social_post_date >= 2025-12-21 minus 365 days (active social presence in the last year).';
...
ALTER TABLE workspace.virtue_foundation_enriched.facilities_gold ALTER COLUMN has_operating_hours COMMENT 'opd_hours_final IS NOT NULL: TRUE when any operating-hours text was recovered from the OPD blob or the description.';
