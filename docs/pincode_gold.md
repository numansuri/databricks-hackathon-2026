# `pincode_gold` — Gold Layer Doc

**Table:** `workspace.virtue_foundation_enriched.pincode_gold` · **Grain:** one row per India Post pincode (**19,586**) · **42 columns**, all with persisted descriptions (`DESCRIBE TABLE …`).
...
## Lineage
```
india_post_pincode_directory   →   pincode_office_silver   →   pincode_gold
...
2. **Gold — roll up to one row per pincode** (`sql/11_pincode_gold.sql`): `GROUP BY pincode` over silver.
...
**NFHS join keys** are `district_majority` + `state_majority` (resolve via the crosswalk / `pincode_nfhs` view). Every column's full description is on the table itself — `DESCRIBE TABLE workspace.virtue_foundation_enriched.pincode_gold`.
...
sql/11_pincode_gold.sql            -- this table (+ its column descriptions)
